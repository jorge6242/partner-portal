...
import { ACTIONS } from '../interfaces/actionTypes/recordTypes';
...
export const getRecordsByPerson = (
  form: object,
  page: number = 1,
  perPage: number = 8
  ...
  try {
    const {
      data: { data },
      status
    } = await API.getRecordsByPerson(form, page, perPage);
    let response = [];
    if (status === 200) {
      const pagination = {
        total: data.total,
      ...
      response = data.data;
      dispatch({
        type: ACTIONS.GET_RECORDS_BY_PERSON,
        payload: response
      });
      dispatch({
        type: ACTIONS.SET_PAGINATION,
