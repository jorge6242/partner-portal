...
import { ACTIONS } from '../interfaces/actionTypes/noteTypes';
...
export const create = (body: any) => async (dispatch: Function) => {
  ...
  try {
    ...
    if (status === 200 || status === 201) {
      ...
      dispatch(getNotesByPerson({id: body.people_id}));
      dispatch(
        updateSecondModal({
          payload: {
            status: false,
...
export const getNotesByPerson = (
  ...
  try {
    const {
      data: { data },
      status
    } = await API.getNotesByPerson(form, page, perPage);
    let response = [];
    if (status === 200) {
      const pagination = {
        total: data.total,
      ...
      response = data.data;
      dispatch({
        type: ACTIONS.GET_NOTES_BY_PERSON,
        payload: response
      });
      dispatch({
        type: ACTIONS.SET_PAGINATION,
